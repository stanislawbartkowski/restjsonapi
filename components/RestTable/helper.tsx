...
import type { TAction, TActions, TTag, TTags, FieldValue, ColumnValue, AddStyle, FormMessage, ClickResult, ColumnFilterSearch } from './types';
...
function constructAction(t: TAction, row: Row, r: TableHookParam): ReactElement {
  return <a onClick={() => clickActionHook(t, row, r)}>{makeMessage(t, row)}</a>
}
...
function transformOneColumn(c: TColumn, r: TableHookParam): ColumnType<any> {
  ...
  if (sort(c)) {
    ...
    switch (fieldtype) {
      ...
      case TYPEBOOLEAN: c.props.sorter = (a: Row, b: Row) => sortboolinc(a, b, c.field); break;
      default: c.props.sorter = (a: Row, b: Row) => sortinc(a, b, c.field); break;
    }
  }

  const filterprops: ColumnFilterSearch | undefined = filter(c) ? TableFilterProps(c, mess) : undefined;
